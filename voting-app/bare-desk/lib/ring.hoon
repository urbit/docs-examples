...
++  detail
  ...
  ++  build-signing-participants
    |=  [our=@p now=@da invited=(list [=ship =life])]
    ^-  (set @udpoint)
    ::
    =|  keys=(set @udpoint)
    ::
    ...
    ::
    $(invited t.invited, keys (~(put in keys) pubkey))
  ::
  ::
...
++  sign
  ...
  ::
  =.  members  (~(put in members) [our our-life])
  ::
  ...
  ::
  =/  keys=(set @udpoint)
    (build-signing-participants:detail our now ~(tap in members))
